...
ko.extenders.imageGrid = function(target, options){
	...
	/*
		Sort of an odd fix here, but initially the window will have no scrollbar in the browser as the images
		have yet to be rendered. In order to ensure the template renders the scroll bar, I add in a minimum height to the
		image grid. This way the grid renders once, triggers the browser scrollbar, and then the width of the area will take
		the scrollbar into account the first time.
		works in ie7+
	*/
	target.containerMinHeight = ko.observable($(window).height() + 1 + 'px');
	...
	target.makeGrid = function(){
		if(!target.calculating() && options.container.innerWidth() > 0){
